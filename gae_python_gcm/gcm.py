...
GOOGLE_GCM_SEND_URL = 'http://fcm.googleapis.com/fcm/send' if LOCALHOST \
else 'https://fcm.googleapis.com/fcm/send'
GOOGLE_GCM_SEND_URL = 'http://fcm.googleapis.com/fcm/send' if LOCALHOST \
else 'https://fcm.googleapis.com/fcm/send'
...
#            success = resp_json['success']
            ...
            if failure == 0 and canonical_ids == 0:
                ...
                if 'on_success_callback_func' in GCM_CONFIG:
                    success_callback_func_path = GCM_CONFIG['on_success_callback_func']
                    mod_path, func_name = success_callback_func_path.rsplit('.', 1)
                    mod = importlib.import_module(mod_path)

                    logging.info('success_callback_func_path: ' + repr((mod_path, func_name, mod)))

                    success_callback_func = getattr(mod, func_name)

                    success_callback_func(message, resp_json)
                return
            else:
                ...
                for result in results:
                    ...
                    if 'message_id' in result and 'registration_id' in result:
                        ...
                        try:
                            ...
                            if 'on_error_callback_func' in GCM_CONFIG:
                                error_callback_func_path = GCM_CONFIG['on_error_callback_func']
                                mod_path, func_name = error_callback_func_path.rsplit('.', 1)
                                mod = importlib.import_module(mod_path)

                                logging.info('error_callback_func_path: ' + repr((mod_path, func_name, mod)))

                                error_callback_func = getattr(mod, func_name)

                                error_callback_func(message, resp_json)
                        except:
